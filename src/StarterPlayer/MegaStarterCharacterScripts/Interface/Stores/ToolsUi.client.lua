...
function Ui.updateDisplay(name, info, owned, force, ignoreVariants)
	...
	if displayConn then displayConn:Disconnect() end
	...
	infoFrame.Damage.Value.Text = math.round(info.Damage.Damage * (hitMults and (hitMults.Player or 1)) or 1) 
...
function Ui.updateToolScroller(target)
	...
	for _, tName in pairs(sortRef) do
		...
		local locked = not owned and next(Table.Difference(tTycoonInfo.Dependencies or {}, playerTools))
...
local function showUi()
	...
	while canvas.Visible and spawnerVal.Value do
		local distance = LocalPlayer:DistanceFromCharacter(spawnerVal.Value.PromptPart.Position)
