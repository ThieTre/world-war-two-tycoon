...
local prizeSounds = LocalPlayer.PlayerGui.Monetization:WaitForChild("ChestOpen"):WaitForChild('Sounds')
...
local function promptProduct(id)
	...
	promptConn = MarketService.PromptProductPurchaseFinished:Connect(function(userId, productId, isPurchased)
...
function Ui.setPrizeTile(name: string, prizeInfo: {}, tile: GuiObject)
	...
	if prizeInfo.Type == 'Minutes Reward' then
		displayName = '$'..TyUtils.formatNumber(Spinner.getRoundedMinutesReward(LocalPlayer, prizeInfo.Value))
	end
	...
	table.insert(connections, tile.Button.MouseButton1Click:Connect(function()
...
function Ui.spin(): string
	...
	local success, err = pcall(function()
		...
		local flashTween = TweenService:Create(content.Flash, TweenInfo.new(0.3), {BackgroundTransparency=0.6})
		flashTween:Play()
		flashTween.Completed:Connect(function()
			TweenService:Create(content.Flash, TweenInfo.new(0.3), {BackgroundTransparency=1}):Play()
		end)
		...
		local scaledSize = UDim2.fromScale(origionalSize.X.Scale*1.3, origionalSize.Y.Scale*1.3)
		local sizeTween1 = TweenService:Create(selectedTile, TweenInfo.new(0.3), {Size=scaledSize})
		local sizeTween2 = TweenService:Create(selectedTile, TweenInfo.new(0.3), {Size=origionalSize})
