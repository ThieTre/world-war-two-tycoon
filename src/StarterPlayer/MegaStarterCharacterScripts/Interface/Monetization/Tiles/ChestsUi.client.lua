			...
			purchaseConn = MarketService.PromptProductPurchaseFinished:Connect(function(_uid, productId, isPurchased)
				...
				if productId == info.Id then
					...
					for i in MiscUtils.count() do
						...
						if i > 5/0.01 then
							LOG:Error('Failed to find purchased chest for user' .. LocalPlayer.UserId)
							break
						end
						task.wait(0.01)
					end
...
function Ui.openChest(name)
	...
	context:Show(openCanvas, 1, {EnterStyle='Fade'})
	task.wait(0.2)
	...
	local sizeTween = TweenService:Create(image, TweenInfo.new(1.8), {Size=scaledSize})
	sizeTween:Play()
	...
	local flashTween = TweenService:Create(openCanvas.Flash, ftInfo, {BackgroundTransparency=0.6})
	flashTween:Play()
	flashTween.Completed:Connect(function()
		TweenService:Create(openCanvas.Flash, ftInfo, {BackgroundTransparency=1}):Play()
	end)
